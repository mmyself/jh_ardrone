...
#ifdef __PF_TEST
void ardrone_particle_filter::perform_motion_update2(ardrone_state& drone_state)
{
  //For each particle in vec_pf2 perform a motion update
  //Compute the prior probability
  vector<ardrone_particle>::iterator it=vec_pf2->begin();
...
void ardrone_particle_filter::perform_measurement_update(const ar_recog::Tags::ConstPtr& tagsmsg,vector<tagPose> &tagposes)
{
  ...
  if(tagsmsg->tags.size()>0)
      ...
      if(biggestDiameter>0)
	{
	  ...
	  /*
	  double rzrot = tagsmsg->tags[biggestTag].zRot;
	  double tthz=0.0;
	  if(rzrot>0)
	    {
	      if(tc[2]>tc[0])
	  ...
	  for(;it<vec_pf->end();it++)
	    {
	      ...
	      //computing likelihood of drone yaw orientation
	      //assuming that the error in yaw orientation is not greater than 2PI
	      //Need to account for the case when drone_rpy[2] and dronetrueyaw have -
	      //different signs
	      double yaw_error=0.0;
	      if(((*it).drone_rpy[2]*dronetrueyaw)<0)
		{
		  if((fabs((*it).drone_rpy[2])>(PI/2.0))&&(fabs(dronetrueyaw)>(PI/2.0)))
		    {
		      yaw_error = fabs(((*it).drone_rpy[2]-dronetrueyaw));
